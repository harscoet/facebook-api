...
function isFemale(user) {
    return (user.gender === types_1.Gender.female_plural ||
        user.gender === types_1.Gender.female_singular ||
        user.gender === types_1.Gender.female_singular_guess);
}
exports.isFemale = isFemale;
function isMale(user) {
    return (user.gender === types_1.Gender.male_plural ||
        user.gender === types_1.Gender.male_singular ||
        user.gender === types_1.Gender.male_singular_guess);
}
...
function findFromCodeTags(htmlStringOrHtmlDom, selector) {
    ...
    let $container;
    $codes.forEach($code => {
        const $candidateContainer = parseCommentedHtmlFromString($code.innerHTML).querySelector(selector);
