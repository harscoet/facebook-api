...
export function getThreadPicture(request: FacebookRequest) {
  return async (
    threadId: string,
    imageId: string,
  ): Promise<GetThreadPicture.Response> => {
    ...
    // Ugh...
    const queryThreadID =
      result.jsmods.require[0][3][1].query_metadata.query_path[0]
        .message_thread;
    const imageData =
      result.jsmods.require[0][3][1].query_results[queryThreadID].message_images
        .edges[0].node.image2;
